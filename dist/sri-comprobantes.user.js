...
(function() {
	...
	(function() {
		...
		async function refreshReportStatusFromApi(force = false) {
			...
			state.receiverRuc = receiverRuc;
			try {
				const url = `${CONFIG.API_BASE}${CONFIG.API_REPORT_STATUS_PATH}?receiverRuc=${encodeURIComponent(receiverRuc)}&year=${encodeURIComponent(year)}&month=${encodeURIComponent(month)}&day=${encodeURIComponent(day)}&documentType=${encodeURIComponent(documentType)}`;
		...
		function getCurrentPageRuc() {
			if (CONFIG.RECEIVER_RUC_OVERRIDE) return onlyDigits(CONFIG.RECEIVER_RUC_OVERRIDE);
		...
		function toApiReceiverRuc(value) {
			...
			if (CONFIG.SEND_RECEIVER_AS_BASE10_WHEN_ENDS_001 && digits.length === 13 && digits.endsWith("001")) return digits.slice(0, 10);
			return digits;
		}
		...
		function getCurrentPeriodsKey() {
			...
			const year = getSelectedYear();
			const documentType = getSelectedDocumentType();
			if (!receiverRuc || !year || !documentType) return null;
		...
		function buildMonthTitle(metadata) {
			return `Estado: ${metadata.status || "-"}. Total: ${metadata.total ?? 0}. Descargadas: ${metadata.downloaded ?? 0}. Pendientes: ${metadata.missing ?? 0}.`;
		}
    ...
    `.trim();
		...
		function downloadTxtForce(customMessage = "Descargando TXT forzado del periodo actual...") {
			...
			setTimeout(() => {
				...
				updateTxtButtons();
				updateDashboardStats();
			}, Math.max(...CONFIG.REFRESH_AFTER_DOWNLOAD_MS) + 1e3);
		}
		...
		function processCurrentPageForBatch() {
			...
			updateDashboardStats();
			if (queue.length === 0) {
				if (state.batchAcrossPages && moveToNextPageAndContinue()) return;
				finishBatch(`Descarga finalizada. Archivos procesados: ${state.batchDownloadedCount}.`, "success");
				return;
			}
			renderDashboard({
				status: "loading",
		...
		function processDownloadQueue(queue) {
			...
			renderDashboard({
				status: "loading",
				message: `Descargando ${item.file.toUpperCase()} ${state.batchDownloadedCount}/${state.batchQueueTotal}. Restantes en página: ${queue.length}.`
			});
			item.link.click();
			for (const delay of CONFIG.REFRESH_AFTER_DOWNLOAD_MS) setTimeout(() => {
				refreshInvoicesFromApi(true);
		...
		function moveToNextPageAndContinue() {
			...
			renderDashboard({
				status: "loading",
				message: `Avanzando a la siguiente página desde ${getPaginationSignature()}...`
			});
			nextButton.click();
			setTimeout(async () => {
		...
		function getPaginationSignature() {
			...
			if (current) return normalizeSpaces(current.textContent || "");
			const tbody = findComprobantesTbody();
			if (!tbody) return "sin-tabla";
			return Array.from(tbody.querySelectorAll("tr")).map((row) => row.textContent || "").join(" ").match(/\b\d{49}\b/)?.[0] || `pagina-${Date.now()}`;
		}
    ...
    `;
		...
		function updateDashboardStats() {
			...
			setText("tm-sri-meta-period", `${getSelectedYear() || "-"} / ${getSelectedMonth() || "-"} / día ${getSelectedDay()}`);
			setText("tm-sri-meta-hidden-months", state.hiddenMonthsCount);
			setText("tm-sri-meta-txt", report ? `${report.status || "-"} | ${report.should_download_txt ? "descargar" : "ok"} | ${report.reason || "-"}` : state.lastReportStatusError ? `error: ${state.lastReportStatusError}` : "-");
			setText("tm-sri-meta-last-refresh", state.lastInvoicesRefreshAt ? state.lastInvoicesRefreshAt.toLocaleTimeString() : "-");
			setText("tm-sri-meta-batch", state.isBatchDownloading ? `${state.batchDownloadedCount}/${state.batchQueueTotal} | páginas: ${state.batchPageCount}` : "Inactivo");
			updateFilterButtons();
			updateCompactButton();
			updateBatchButtons();
			updateTxtButtons();
