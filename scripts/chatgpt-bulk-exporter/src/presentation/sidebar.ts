...
export function mountSidebar(): void {
  ...
  let field: DateField = 'updated'; let filterOpen = false; let exportSummary: { exported: number; failed: number } | null = null; let syncController: AbortController | null = null; let syncStatus = '';
  ...
  const footer = document.createElement('div'); footer.className = 'cbe-popover-footer'; const cancel = document.createElement('button'); cancel.type = 'button'; cancel.className = 'cbe-secondary-button'; cancel.textContent = 'Cancelar'; const exportButton = document.createElement('button'); exportButton.type = 'button'; exportButton.className = 'cbe-primary-button'; footer.append(cancel, exportButton); const syncActions = document.createElement('div'); syncActions.className = 'cbe-sync-actions'; const chooseFolder = document.createElement('button'); chooseFolder.type = 'button'; chooseFolder.className = 'cbe-secondary-button'; chooseFolder.textContent = 'Elegir carpeta ChatGPT'; const syncNow = document.createElement('button'); syncNow.type = 'button'; syncNow.className = 'cbe-secondary-button'; syncNow.textContent = 'Sincronizar ahora'; syncActions.append(chooseFolder, syncNow); popover.append(header, status, filterToggle, filterPanel, actions, list, empty, syncActions, footer); root.append(overlay, popover); document.body.append(root);
  ...
  const syncNowWithFolder = async (handle: FileSystemDirectoryHandle) => { syncController = new AbortController(); syncStatus = 'Sincronizando carpeta…'; refresh(); try { const history = conversations.length ? conversations : await fetchConversationHistory({ signal: syncController.signal }); const result = await syncConversations({ handle, conversations: history, fetchConversation: fetchConversationForExport, signal: syncController.signal, now: new Date() }); syncStatus = `Carpeta ChatGPT: ${syncSummary(result)}`; } catch (caught) { if (!(caught instanceof DOMException && caught.name === 'AbortError')) syncStatus = caught instanceof Error ? `No se pudo sincronizar: ${caught.message}` : 'No se pudo sincronizar la carpeta.'; } finally { syncController = null; refresh(); } };
  chooseFolder.addEventListener('click', async () => { try { const handle = await chooseDirectory(); await syncNowWithFolder(handle); } catch (caught) { syncStatus = caught instanceof Error ? caught.message : 'No se pudo elegir la carpeta.'; refresh(); } });
  syncNow.addEventListener('click', async () => { try { const handle = await getSavedDirectory(); if (!handle) { syncStatus = 'Primero elige una carpeta ChatGPT.'; refresh(); return; } await syncNowWithFolder(handle); } catch (caught) { syncStatus = caught instanceof Error ? caught.message : 'No se pudo acceder a la carpeta.'; refresh(); } });
  void getSavedDirectory().then(async handle => { if (handle && await hasDirectoryPermission(handle)) await syncNowWithFolder(handle); }).catch(() => undefined);
