...
export function mountSidebar(): void {
  ...
  const trigger = mountSelectionTrigger(target, () => { const rect = trigger.getBoundingClientRect(); selecting = true; popover.hidden = false; overlay.hidden = false; trigger.hidden = true; trigger.setAttribute('aria-expanded', 'true'); const width = Math.min(390, window.innerWidth - 24); const maxHeight = Math.min(window.innerHeight * 0.78, 720); const left = Math.max(12, Math.min(rect.left, window.innerWidth - width - 12)); const below = rect.bottom + 8; const top = below + maxHeight <= window.innerHeight - 12 ? below : Math.max(12, rect.top - maxHeight - 8); popover.style.width = `${width}px`; popover.style.maxHeight = `${maxHeight}px`; popover.style.left = `${left}px`; popover.style.top = `${top}px`; void loadHistory(); }); trigger.setAttribute('aria-controls', 'cbe-export-popover'); popover.id = 'cbe-export-popover';
